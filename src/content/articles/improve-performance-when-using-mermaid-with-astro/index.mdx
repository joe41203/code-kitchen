...
pubDate: '2024-03-15'
cover: './hero.webp'
tags: [Astro, Mermaid, PageSpeedInsights]
---
...
```html
<script is:inline src="https://cdn.jsdelivr.net/npm/mermaid/dist/mermaid.min.js" defer></script>
```
...
graph TB
  A[Astro] --> B[mermaid]
  B --> C{Page Speed Insights}
  C -->|Low Score| D[SVG Conversion]
  C -->|High Score| E[Optimized Performance]
```
````

上記のコードは、以下のようなフローチャートを生成します。
...
このように、mermaid記法を使用することで、簡単に図を描画することができます。

### クラス定義によるスタイル適用

mermaidで生成されるSVGにスタイルを適用するには、クラス定義を使用します。以下は、クラス定義の例です。
...
上記のコードでは、`classDef`を使用して`default`と`optimized`の2つのクラスを定義しています。`default`クラスは、塗りつぶしに`#f9f`、線に`#333`、線の太さに`2px`を指定しています。`optimized`クラスは、塗りつぶしに`#9f9`を指定しています。

次に、`class`を使用して、各ノードにクラスを適用しています。`A`、`B`、`C`、`D`、`E`のノードには`default`クラスを適用し、`E`ノードには`optimized`クラスを適用しています。

これにより、以下のようなスタイルが適用されたフローチャートが生成されます。
...
このように、クラス定義を使用することで、生成されるSVGにスタイルを適用することができます。

以上が、mdxファイルでmermaidを使用する際の記述方法です。コードブロックでmermaid記法を使用し、クラス定義でスタイルを適用することで、見やすく分かりやすい図を作成することができます。
