...
3. ミドルウェアは、設定に基づいてUser-Agentを選択します。
  - デフォルト：scrapy-fake-useragentライブラリに含まれるデフォルトのUser-Agentリストから選択します。
...
Scrapy Fake User Agentは、内部でscrapy-fake-useragentライブラリを活用しています。このライブラリは、実在のUser-Agent情報を提供し、ランダムな選択やローテーションを可能にします。
